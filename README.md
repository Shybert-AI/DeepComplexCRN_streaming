...
- 已按照 Apache-2.0 许可证的要求，保留原始版权声明和许可证副本。原始许可证文本见 [LICENSE](./LICENSE) 文件。
- 我对本项目中**新增的流式推理功能代码**（主要涉及 `predict_forward_streaming.py` 等文件）拥有完整的著作权。
- 本项目作为衍生作品，**整体以闭源形式提供商业授权**。任何使用、修改或分发请遵守 Apache-2.0 许可证及本商业授权条款。
- 流式推理的实现涉及复杂的状态管理和实时优化，需要大量研发投入。因此，本项目以商业合作形式提供完整解决方案，以维持可持续开发。
...
本项目**提供 Python 推理/训练代码**。如需以下服务，欢迎洽谈：
